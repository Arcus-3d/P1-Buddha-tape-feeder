	...
	flat=false)
{
	...
	// Pitch diameter: Diameter of pitch circle.
	pitch_diameter  =  fake_teeth * circular_pitch / 180;
	pitch_radius = pitch_diameter/2;
	echo ("Teeth:", number_of_teeth, " Pitch radius:", pitch_radius);
	...
	// Diametrial pitch: Number of teeth per unit length.
	pitch_diametrial = fake_teeth / pitch_diameter;
	...
	root_radius = pitch_radius-dedendum;
	backlash_angle = backlash / pitch_radius * 180 / pi;
	half_thick_angle = (360 / fake_teeth - backlash_angle) / 4;
	...
	difference()
	{
		union ()
		{
			...
			{
				linear_exturde_flat_option(flat=flat, height=rim_thickness, convexity=10, twist=twist)
				for(i=[0:number_of_teeth]) translate([-root_radius,diametral_pitch*i]) union() {
					intersection() {
						involute_gear_tooth (
							pitch_radius = pitch_radius,
							root_radius = root_radius,
							...
							half_thick_angle = half_thick_angle,
							involute_facets=involute_facets);
						translate([root_radius,-diametral_pitch/2]) square([root_radius,diametral_pitch]);
					}
					translate([root_radius-rack_thickness,-diametral_pitch/2]) square([rack_thickness,diametral_pitch]);
				}
			}
		}
	}
	...
	involute_facets)
{
	...
	union () for (i=[1:res]) {
		...
		side2_point2=mirror_point (rotate_point (centre_angle, point2));
		polygon (
			points=[[0,0],side1_point1,side1_point2,side2_point2,side2_point1], paths=[[0,1,2,3,4,0]]);
	}
}
